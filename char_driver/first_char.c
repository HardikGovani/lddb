...
MODULE_DESCRIPTION("A simple example Linux module/char_device.");
MODULE_VERSION("0.01");
#define DEVICE_NAME "first_char"
...
static int __init first_char_init(void) {
 ...
 major_num = register_chrdev(0,"first_char", &file_ops);
 if (major_num < 0) {
	 printk(KERN_ALERT "Could not register device: %d\n", major_num);
 return major_num;
 } else {
	 printk(KERN_INFO "first_char  module loaded with device major number %d\n", major_num);
 return 0;
 }
}
