/*
 * char_full.c originally modified from chardev.c: Creates a read-only char device that says how many times you've
 ...
 *
 * Compile with `make`. Load with `sudo insmod char_full.ko`. Check `dmesg | tail`
...
/*
 * Called when a process writes to dev file: echo "hi" > /dev/char_full
 */
static ssize_t
